...
def print_with_var_dump(file_name, data):
    vacancies_objects = []
    if data == []:
        var_dump(DataSet(file_name, data))
        sys.exit()
    for x in data:
        salary = Salary(str(float(x['salary_from'])), str(float(x['salary_to'])), x['salary_gross'],
                        x['salary_currency'])
        vacancy = Vacancy(x['name'], x['description'], x['key_skills'].split('\n'), x['experience_id'], x['premium'],
                          x['employer_name'], salary, x['area_name'], x['published_at'])
...
print_with_var_dump(file_name, resume_list)
